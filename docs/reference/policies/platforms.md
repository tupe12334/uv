...
For glibc-based Linux distributions, uv publishes [manylinux-compatible](https://peps.python.org/pep-0600/)
wheels and corresponding binaries. These artifacts depend on glibc being available on the host
system. In a manylinux wheel tag, the version encodes the minimum supported glibc version for that
wheel; for example, `manylinux_2_17_x86_64` means glibc 2.17+ on x86_64.
...
For musl-based Linux distributions, such as Alpine, uv publishes wheels and fully statically
linked musl binaries for the following targets:
...
However, the embedded `uv` binaries are fully statically linked and do not require musl libc on
the host system.
